...
module.exports = function(child, parents, percentPlayed) {
  ...
  const layers = parents.slice(1).concat([child]);
  layers.forEach(layer => {
    const offsetScale = scale / layer.parent.offsetScale;
    x += (getInterpolatedValue(layer.x, percentPlayed) - layer.parent.offsetX) * offsetScale;
    y += (getInterpolatedValue(layer.y, percentPlayed) - layer.parent.offsetY) * offsetScale;
    scale *= getInterpolatedValue(layer.scale, percentPlayed) / layer.parent.offsetScale;
